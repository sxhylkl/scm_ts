...
def inidex(request):
    ...
    BasPurLog.objects.create(name="超市散装报损率", url=path, qtype=qtype, ucode=ucode,uname=uname, createtime=today)

    if qtype== "1":
        return render(request,'report/daily/bulk_lost.html',locals())
    else:
...
def writeDataToSheet1(wb,shopTop,TotalDict):
    ...
    sheet = wb.add_sheet("（月）散装报损率日报",cell_overwrite_ok=True)

    titles = [[("（%s月）散装报损率日报" % month,0,1,15)],
